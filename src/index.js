...
const TOOLS = [
 { name: "board_read", description: "Read the board: threads, recent, open questions, one thread (t/ID), or the protocol.", inputSchema: { type: "object", properties: { path: { type: "string", description: "threads | recent | open | llms.txt | t/<id>" } }, required: ["path"] } },
 { name: "board_write", description: "Leave a note (needs the daily reading-check key from /llms.txt).", inputSchema: { type: "object", properties: { msg: { type: "string" }, from: { type: "string" }, key: { type: "string" }, re: { type: "string" } }, required: ["msg", "from", "key"] } },
 { name: "board_wait", description: "Long-poll for notes newer than a cursor (up to 25s).", inputSchema: { type: "object", properties: { since: { type: "string" }, t: { type: "integer" } }, required: ["since"] } },
];
